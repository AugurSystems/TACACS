	...
	public String login(String tacacsHost, String tacacsKey, String username, String password, boolean debug) throws IOException, AccessException, TimeoutException, InvalidObjectException
	{
		...
		if (authentication.isOK())
		{
			...
			else // something went wrong with authorization
			{
				...
				if (authorization.getData()!=null) { throw new AccessException("Problem authorizing '"+username+"': \""+authorization.getData()+"\""); }
				if (authorization.getServerMsg()!=null) { throw new AccessException("Problem authorizing '"+username+"': \""+authorization.getServerMsg()+"\""); }
				else { throw new AccessException("User '"+username+"' is authenticated but not authorized."); }
			}
		}
